...
def multihead_attention(queries, keys, values, kernel_initializer=tf.contrib.layers.variance_scaling_initializer(),
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        if key_masks is None and query_masks is None and num_steps > head_value_channels:  # 使用efficient attention加速运算
            ...
            Q, K, outputs = scaled_dot_product_attention(Q_, K_, V_, causality, dropout_rate, training, key_masks,
                                                         query_masks, num_heads=num_heads)
        # Restore shape
...
def linear_scaled_dot_product_attention(Q, K, V,
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        head_value_channels = V.get_shape().as_list()[-1]
        attended_values = []
        for i in range(num_heads):
            ...
            context = tf.matmul(tf.transpose(key, [0, 2, 1]), value)  # (batch_size,k_dim,V_dim)
            attended_value = tf.matmul(tf.transpose(context, [0, 2, 1]),
                                       tf.transpose(query, [0, 2, 1]))  # (batch_size,V_dim,seq_size)
            attended_values.append(attended_value)
        aggregated_values = tf.concat(attended_value, axis=1)  # (batch_size,head_num * V_dim,seq_size)
        aggregated_values = tf.transpose(aggregated_values, [0, 2, 1])
        aggregated_values = tf.concat(tf.split(aggregated_values, num_heads, axis=2),
                                      axis=0)  # (head_num*batch_size,seq_size,V_dim)

        # # dot product    Q(head_num*batch_size,seq_size,dim)         K(head_num*batch_size,dim , seq_size)
        # outputs = tf.matmul(Q, tf.transpose(K, [0, 2, 1]))
        #
        # # scale https://github.com/cmsflash/efficient-attention/blob/master/efficient_attention.py 没有使用
        # # outputs /= d_k ** 0.5 //
        #
        # # key masking
        # if key_masks is not None:
        ...
        #
        # # weighted sum (context vectors)
        # outputs = tf.matmul(outputs, V)  # (h*N, T_q, d_v)
