...
class SA_GAN_SEQ(object):
    ...
    def generator(self, z, is_training=True):
        ...
        with tf.variable_scope("Generator", reuse=tf.AUTO_REUSE):  # z's shape (batch_size, z_dim)
            x = tf.layers.dense(z, units=self.seq_size * self.d_model,kernel_initializer=he_init)  # batch_size, seq_size
            x = tf.nn.relu(x)
    ...
    def self_attention(self, input, is_training=True,scope="multihead_attention"):
    ...
    def discriminator(self, x, is_training=True):
        ...
        with tf.variable_scope("discriminator", reuse=tf.AUTO_REUSE):
            ...
            # output += positional_encoding(output, self.seq_size)
            # output = self.self_attention(output, is_training=is_training)#(batch_size, seq_size, d_model)
            ...
            output2 = self.self_attention(output1, is_training=is_training, scope="multihead_attention_3")
                ...
                for _ in range(self.critic_iters):
                    ...
                    if iter % n_batch == 0:  ## 每一次训练集中的数据训练完一遍的时候
                        ...
                        if (e + 1) % 10 == 0:
                            ...
                            save_gen_samples(data_to_write, self.i2w, e + 1, self.outputdir)  # 保存生成器生成的报文
                            ## 保存时间####
                            ...
                            self.save_final_info(e + 1, "result.txt", write_to_content)
                            ## 保存model e+1 当前的epoch ###
                            self.save(self.outputdir + os.path.sep + "epoch_" + str(e + 1), step)
                            ##画图,保存图片信息######
    ...
    def draw_result_picture(self, total_batch, fig_w_distance, fig_d_loss_trains, fig_g_loss_trains, current_epoch,
                            fig_time):
        save_dir = self.outputdir + os.path.sep + "epoch_" + str(current_epoch)
    ...
    def save_final_info(self, epoch, file_name, content):
        write_folder = self.outputdir + os.path.sep + "epoch_" + str(epoch)
